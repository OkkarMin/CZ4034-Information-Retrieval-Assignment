...
raw_data_directory = "C:/Users/whate/Documents/AY2021S2/CZ4034 Information Retrieval/twitter stuff/TECH_DATA_2/"
...
def process_body(body_str):
    #remove all mentions
    body_str = "".join(
        [" " if "@" in word else word + " " for word in body_str.split()])
    ...
    #remove URLs
    body_str = "".join([
        " " if "https" in word or "//t.co" in word else word + " "
        for word in body_str.split()
    ])
...
company_files_dict = {
    ...
    "adobe": ["Adobe", "Photoshop", "Lightroom", "creativecloud", "AdobeCare"],
    "salesforce": [
...
for company in company_files_dict:
    ...
    try:
        new_dict = {}
        new_dict["data"] = []
        file_name = "{}Data.json".format(company)
        ...
        print("{} entries in {} created!".format(len(new_dict["data"]),
                                                 created_file_name))
        with open(cleaned_data_directory + created_file_name, 'w') as p:
            json.dump(new_dict, p)
    except Exception as e:
        print(e)
        print("[twitterCrawler]: Error in twitterCleanUp.py!")
